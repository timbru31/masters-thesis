...
The following section sums the introduced methods of authentication and analyzed \gls{mfa} solutions up and shows their key threats.

\autoref{tab:all-threats} shows the introduced authentication methods grouped by the authentication methods knowledge, possession, and biometrics. It shows that primary authentication by possession is used as an additional authentication factor, given the fact that passwords are the de facto standard on the internet as the first factor. Biometrics can be used, for instance, with the \gls{uaf}, but in practice there exist very few applications that use biometrics as an additional factor for internet-based login flows.
...
\begin{table}[ht]
	\begin{tabularx}{\textwidth}{c|p{3.5cm}|p{1.5cm}|p{8cm}}
		...
		\cline{2-4}
		& Smart cards & \OK & Cloning, theft, damage, oblivion, loss, side-channel attacks, phishing (in case of \gls{otp} generation) \\
		\cline{2-4}

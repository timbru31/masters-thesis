...
\autoref{fig:possesion_auth_flow} shows an example authentication flow with a smartcard. First, the user inserts the given smartcard into their computer. The data is read subsequently. Contemporaneous the application or system reads the stored database entry and compares the data to the one stored on the smartcard. If the data is equal or matches and the user authorized, then the authentication succeeds. Again any user can log on as long as they are in the possession of the smartcard.

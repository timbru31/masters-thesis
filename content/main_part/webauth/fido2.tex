...
The API defined in the WebAuth is actually an extension of the Credential Management API, which is another API in development, but currently in a draft state. The Credential Management API defines \textit{navigator.credentials} property with the \textit{create} and \gls{get} methods and has the goal to offer an API for programatically accessing the user agent's password storage capabilities. The \wa{} is adding further method overloads to support public-key based credentials, too.
...
\begin{example}[listing:webauthn_reg]{Exemplary Web Authentication API registration}
...
\autoref{listing:webauthn_reg} shows an example payload for the registration of a new credential with the \wa. Noteworthy parameter and options include the \textit{user.id}, which should not be an information that could lead to user identification because this ID can be stored on the authenticator.

\begin{example}[listing:webauthn_reg_resp]{Web Authentication API registration response}
\begin{minted}[breaklines]{javascript}
const credential = {
...
\autoref{listing:webauthn_reg_resp} shows the received response from the creation generation, i.e., the newly generated credentials for the \gls{rp}.

\begin{example}[listing:webauthn_reg_resp_client_data]{Web Authentication API registration client data}
\begin{minted}[breaklines]{javascript}
...
\autoref{listing:webauthn_reg_resp_client_data} shows the decoded client data from the \wa{} registration response from \autoref{listing:webauthn_reg}. The data contains the challenge sent by the \gls{rp} and the origin of the \gls{rp}. Each registration is flagged with the type of \texttt{webauthn.create}.

\begin{example}[listing:webauthn_reg_resp_attestation]{Web Authentication API registration attestation}
\begin{minted}[breaklines]{javascript}
...
\begin{example}[listing:webauthn_auth]{Exemplary Web Authentication API authentication}
\begin{minted}[breaklines]{javascript}
const publicKeyOptions = {
...
\begin{example}[listing:webauthn_auth_resp]{Web Authentication API authentication response}
\begin{minted}[breaklines]{javascript}
const assertion = {
  id: 'BShOCQ2c32dv4aqyy3oWmcu_9s4tz0VIob81U5tg [...]',
  rawId: ArrayBuffer(59),

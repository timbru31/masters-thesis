...
\autoref{fig:2fa_flow_ss7} again the described \gls{mfa} flow using \gls{totp}. In this scenario the attacker is still able to phis the \gls{totp} designated for the user. The figure shows that the attacker uses an exploit in the \gls{ss7} network. This allows them to intercept all incoming \glspl{sms}.

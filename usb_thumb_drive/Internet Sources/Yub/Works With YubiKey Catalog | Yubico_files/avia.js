...
{	
    ...
    function avia_responsive_menu()
    {
    	...
    	var menu 			  	= header.find('.main_menu ul:eq(0)'),
	    	...
	    	menu2				= header.find('.sub_menu ul:eq(0)'),
	    	first_level_items 	= menu2.find('>li').length,
	    	langsel 			= $('<span id=header_langs><span id=globe class=av-icon-char aria-hidden=true data-av_icon= data-av_iconfont=globe></span><select onchange=window.location.href=this.value><option label=English value=/></option><option label=Swedish value=/se//></option><option label=German value=/de//></option><option label=French value=/fr//></option></select></span>'),
	    	container			= $('#wrap_all'),
    		show_menu_btn		= $('#advanced_menu_toggle'),
    		hide_menu_btn		= $('#advanced_menu_hide'),
    		mobile_advanced 	= menu.clone().attr({id:"mobile-advanced", "class":""}),
    		mobile_advanced2 	= menu2.clone().attr({id:"mobile-advanced2", "class":""}),
			...
			{	
				...
				else
				{
					...
					mobile_advanced2.appendTo($("#mobile-advanced"));
					langsel.appendTo($("#mobile-advanced"));
...
(function($){
		$(document).ready(function(){
			...
			$('li.menu-item').click( function( event ) {
				var ulchild = $( this ).children('ul.sub-menu');
				$( ulchild ).toggleClass('smshow');
				$( this ).toggleClass('parentover');
				$('.advanced_menu_hide').toggle();
			} );
			//faqs
			$('div#faqq').click( function( event ) {
 				var kids = $( event.target ).children();
				$(kids).toggleClass('hide-xivic');} );
		});
		var mouse_is_inside = false;
		$(document).ready(function()
		{
    	$('#mobile-advanced').hover(function(){ 
        ...
        if(! mouse_is_inside) {
        	...
        	$('#wrap_all').css({'height':"auto", 'overflow':'hidden', 'minHeight':0}); 
			$('#mobile-advanced').css({display:'none'}); }
    });
});
	})( jQuery );

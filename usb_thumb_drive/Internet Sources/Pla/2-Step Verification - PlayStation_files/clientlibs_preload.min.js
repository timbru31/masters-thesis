...
if(h!=null){if(h.match("SCEA")&&a.cookie.match("ag_US=")){var g=getCookie("ag_US");
if(g=="m"){e=true
}else{e=false
}}else{if(h.match("BR")&&a.cookie.match("ag_BR=")){var g=getCookie("ag_BR");
if(g=="m"){e=true
}else{e=false
}}else{if(h.match("LATAM")&&a.cookie.match("ag_MX=")){var g=getCookie("ag_MX");
if(g=="m"){e=true
}else{e=false
}}}}}}return e
}
...
PDC.Commerce=(function eCommerceMarketing(t,w){var e="smcid",p="emcid",i,u=getCookie("sc_emcid"),l=getCookie("sc-cmp-id"),d=getCookie("sc-cmp-type");
...
}function q(y){var x=/^(https?\:)?(\/{2})(vue\.(playstation|sonyentertainmentnetwork))/;
return(y&&x.test(y))
...
F=o(B.attributes.productName,(A)?A.productInfo.productName:null);
F=F.replace(/[^\w\s]/gi,"");
...
}function o(){var x=arguments.length,y;
for(y=0;
y<x;
...
if(c(C)||q(C)){y=t.URI.parse(C);
z=y.queryObject;
if(!z.hasOwnProperty(e)){z[e]=j(A,C,h());
if((!u||u!=i)||(A.getAttribute("target")=="_self"||!A.getAttribute("target"))){if(typeof i!="undefined"){z[p]=i
...
}function n(z){var y=t.Element.convertJQueryElement(z),x;
...
if(z[0].toLowerCase()===x[0].toLowerCase()){return n(y)
}return y
...
var a="sc-cmp-type";
createCookie(h[1]);
if($.cookie){$.cookie(d,h[1],{domain:g});
$.cookie(a,"emcid",{domain:g})
...
function isVideoMature(e,c,f){var d=["rated mature","content inappropriate for children","inappropriate content for children","rated teen-mature","rated teen - mature","rated e-mature","rated e - mature"];
var a=false;
var b="https://www.googleapis.com/youtube/v3/videos?part=snippet,contentDetails&id="+e+"&key=AIzaSyD_v2XHR1lBThUSebq5rKY2v8OgLX42JPc&alt=json&callback=?";
jQuery.ajax({type:"POST",url:b,dataType:"jsonp",success:function(g){if(typeof g.pageInfo!="undefined"&&g.pageInfo.totalResults=="1"){if(typeof g.items!="undefined"){$.each(g.items,function(h,k){var m=k.id;
var l=k.snippet.description;
if(l){var n=l.toLowerCase();
for(var j=0;
j<d.length;
j++){if(n.indexOf(d[j])>=0){a=true
}}}if(typeof window.autoPlayVideoPlugin!=="undefined"){window.autoPlayVideoPlugin(m,a,c,f)
}})
}}}})
}$(document).ready(function(){var g={wwwdev:"https://dev.io.playstation.com/",wwwqa:"https://qa.io.playstation.com/",wwwqa2:"https://qa2.io.playstation.com/",wwwstage:"https://stage.io.playstation.com/",www:"https://io.playstation.com/",preview:"https://preview.io.playstation.com/"};
window.getEnvironment=function(){var h=window.location.hostname;
return h.split(".")[0]
};
environmentVariable=getEnvironment();
var c=g[environmentVariable]+"api/personalization?uh=";
var e="";
if($.cookie("uh")){e=c+$.cookie("uh")
}else{e=c
}var f=typeof $.cookie("userinfo")==="undefined"||$.cookie("userinfo")===null||$.cookie("userinfo")==="";
var b=typeof $.cookie("dcm")==="undefined"||$.cookie("dcm")===null||$.cookie("dcm")==="";
var d=typeof $.cookie("dcm2")==="undefined"||$.cookie("dcm2")===null||$.cookie("dcm2")==="";
var a=$.cookie("hcm")==null;
if(!f&&b){$.ajax({type:"GET",url:e,success:function(i){var h=i.console+"|"+i.psvr+"|"+i.firstGenre+"|"+i.secondGenre+"|"+i.segment+"|"+i.psplus_expDate;
$.cookie("dcm",h,{expires:1,path:"/",domain:location.hostname});
$.cookie("dcm2",h,{expires:365,path:"/",domain:location.hostname})
}})
}else{if(f){$.removeCookie("dcm",{path:"/",domain:location.hostname})
}}if(a){$.cookie("hcm","",{expires:1825,path:"/",domain:location.hostname})
}});
